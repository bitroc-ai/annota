...
export interface MaskLoaderOptions {
  ...
  maskType?: 'auto' | 'binary' | 'instance8' | 'instance16' | 'instance_rgb';
}
...
async function loadMask(
  ...
  // Handle instance_rgb explicitly
  if (maskType === 'instance_rgb') {
    return loadMaskRGB(arrayBuffer);
  }
  ...
  if (maskType === 'auto') {
    if (bitDepth === 16) {
      ...
      // For RGB images, auto-detect as instance_rgb (BC Cell API format)
      if (img.ctype === 2 || img.ctype === 6) {
        return loadMaskRGB(arrayBuffer);
      }
