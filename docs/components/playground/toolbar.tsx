...
export type ToolType = 'pan' | 'point' | 'rectangle' | 'polygon' | 'cell-detect' | 'push';
...
export function DemoToolbar({ tool, onToolChange, viewer, layerPanel }: DemoToolbarProps) {
  ...
  const handleClearAll = () => {
    ...
    if (confirm(`Clear all ${annotations.length} annotation(s)?`)) {
      annotations.forEach(annotation => annotator.state.store.delete(annotation.id));
      toast.success('All annotations cleared');
    }
  };

  return (
    <Card className="bg-neutral-900 border-neutral-800 p-2">
      <div className="flex items-center gap-1">
        ...
        <Button
          ...
          className="bg-neutral-800 hover:bg-red-900 text-neutral-200 hover:text-white transition-colors"
