...
import { Layers, GitBranch, Zap, Code2 } from "lucide-react";
...
```mermaid
graph LR
    A[User Click] --> B[Viewer]
    B --> C[Adapter]
    C --> D[Transform coords]
    D --> E[Store query]
    E --> F[Update selection]
    F --> G[Renderer]
    G --> H[Redraw]
