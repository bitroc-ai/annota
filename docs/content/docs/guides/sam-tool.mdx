...
**Option A: Static Hosting (Recommended for Development)**

Place models in your application's public directory:
...
Then reference with absolute paths:

```tsx
const samTool = new SamTool({
  decoderModelUrl: '/models/sam_vit_h_decoder.onnx',
  embedding: await loadNpyEmbedding('/embeddings/image_001.npy'),
  // ...
});
```
...
Host models on a CDN for better performance and caching:

```tsx
const samTool = new SamTool({
  decoderModelUrl: 'https://cdn.yourapp.com/models/sam_vit_h_decoder.onnx',
  embedding: await loadNpyEmbedding('https://cdn.yourapp.com/embeddings/image_001.npy'),
  // ...
});
```

Popular CDN options:
- **Cloudflare R2** - Zero egress fees, great for large files
- **AWS S3 + CloudFront** - Highly scalable
- **Vercel Edge Network** - If using Vercel for hosting
- **Google Cloud Storage** - Good global distribution

**Option C: Self-Hosted Object Storage**

For sensitive data or compliance requirements:

```tsx
const samTool = new SamTool({
  decoderModelUrl: 'https://storage.yourapp.com/models/sam_vit_h_decoder.onnx',
  embedding: await loadNpyEmbedding('https://storage.yourapp.com/embeddings/image_001.npy'),
  // ...
});
```
...
```
static-assets/
├── models/              # ONNX models (rarely change)
│   ├── sam_vit_h_decoder.onnx
│   └── sam_vit_b_decoder.onnx
├── embeddings/          # Image embeddings (change per dataset)
│   ├── dataset1/
│   │   ├── img001.npy
│   │   └── img002.npy
│   └── dataset2/
...
- **Models**: Long cache (1 year) - these rarely change
  ```
  Cache-Control: public, max-age=31536000, immutable
  ```

- **Embeddings**: Medium cache (1 week) - may update with new data
  ```
  Cache-Control: public, max-age=604800
  ```

- **Use versioned URLs** when updating models:
  ```tsx
  decoderModelUrl: '/models/sam_vit_h_decoder_v1.onnx'
  // Update to v2 when model changes
  ```
...
For local development, place files in `public/` directory:

```tsx
// vite.config.ts or next.config.js handles /public automatically
const samTool = new SamTool({
  decoderModelUrl: '/models/sam_vit_h_decoder.onnx',
  embedding: await loadNpyEmbedding('/embeddings/slide_001.npy'),
  imageWidth: 2048,
  imageHeight: 1536,
});
```
...
```tsx
// .env.production
VITE_MODELS_CDN=https://cdn.yourapp.com
VITE_EMBEDDINGS_CDN=https://cdn.yourapp.com

// App code
const samTool = new SamTool({
  decoderModelUrl: `${import.meta.env.VITE_MODELS_CDN}/models/sam_vit_h_decoder.onnx`,
  embedding: await loadNpyEmbedding(
    `${import.meta.env.VITE_EMBEDDINGS_CDN}/embeddings/${imageId}.npy`
  ),
...
### Hybrid Setup (Model on CDN, Embeddings Local)

Large models on CDN, smaller embeddings bundled with app:

```tsx
const samTool = new SamTool({
  decoderModelUrl: 'https://cdn.yourapp.com/models/sam_vit_h_decoder.onnx',
  embedding: await loadNpyEmbedding('/embeddings/local_slide.npy'),
  imageWidth: 2048,
  imageHeight: 1536,
});
```
...
Load different models based on use case:

```tsx
function getSamModelUrl(accuracy: 'high' | 'balanced' | 'fast') {
  const models = {
    high: '/models/sam_vit_h_decoder.onnx',    // ~10MB, best quality
    balanced: '/models/sam_vit_l_decoder.onnx', // ~6MB
    fast: '/models/sam_vit_b_decoder.onnx',     // ~4MB, fastest
  };
  return models[accuracy];
}

const samTool = new SamTool({
  decoderModelUrl: getSamModelUrl('balanced'),
  embedding,
  imageWidth,
  imageHeight,
});
