...
function EventLogger() {
  ...
  useEffect(() => {
    ...
    const handlers = {
      create: (ann) => setEvents(prev => [...prev, { type: 'create', id: ann.id, time: new Date() }]),
      update: (ann) => setEvents(prev => [...prev, { type: 'update', id: ann.id, time: new Date() }]),
      delete: (ann) => setEvents(prev => [...prev, { type: 'delete', id: ann.id, time: new Date() }]),
    };
...
- [API Reference](/api/annotator) - Annotator instance API
