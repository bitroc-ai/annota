# Image Overlays for Analysis Results

Image shapes allow you to display AI model predictions, heatmaps, and analysis results as persistent overlays on your slide images. This is particularly useful in pathology workflows where you want to visualize region-specific analysis outputs.

## Use Case: Pathology Analysis Workflow

A typical workflow for displaying analysis results:

1. User selects a region of interest (ROI) on the slide
2. Backend analyzes the region (AI model inference, image processing, etc.)
  ...
  const handleAnalyze = async () => {
    ...
    const annotation = {
      ...
      properties: {
        ...
        cellCount: result.metadata.cellCount,
        timestamp: Date.now(),
      },
    };
...
## Displaying Analysis Metadata

You can display the analysis metadata in a popup when users click on the image overlay:
    ...
    <AnnotationPopup>
      <div className="p-4 space-y-2">
        <h3 className="font-semibold">
          {properties?.analysisType || 'Analysis Result'}
        </h3>
...
Image annotations support basic repositioning (drag to move) but have fixed dimensions since they represent analysis output sizes:
...
1. **Keep images small**: Use 256×256 to 640×640 patches for typical analysis regions
2. **Use PNG for masks**: Lossless compression and transparency support
3. **Store metadata**: Use `properties` to store analysis results, timestamps, model versions
4. **Semi-transparent overlays**: Default opacity of 0.6 works well for most use cases
5. **Delete ROI after analysis**: Remove the selection rectangle after creating the result overlay
6. **Layer organization**: Create a dedicated layer for analysis results
