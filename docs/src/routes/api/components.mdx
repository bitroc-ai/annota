...
Context menu component for displaying annotation actions.
...
```tsx
import { ContextMenu, ContextMenuItem, ContextMenuDivider, useContextMenu } from 'annota';
...
function MyContextMenu() {
  ...
  return (
    ...
    >
      <ContextMenuItem onClick={() => handleEdit(contextMenu.annotation)}>
        Edit Annotation
      </ContextMenuItem>
      <ContextMenuItem onClick={() => handleDuplicate(contextMenu.annotation)}>
        Duplicate
      </ContextMenuItem>
      <ContextMenuDivider />
      <ContextMenuItem onClick={() => handleDelete(contextMenu.annotation)}>
        Delete
      </ContextMenuItem>
    </ContextMenu>
  );
...
Individual shape editor components for custom editing interfaces.

### PointEditor

Editor for point annotations.
...
```tsx
import { PointEditor } from 'annota';

<PointEditor
  annotation={selectedAnnotation}
  viewer={viewer}
  onUpdate={(updates) => {
    annotator?.updateAnnotation(selectedAnnotation.id, updates);
  }}
/>
```
...
Editor for rectangle annotations with drag handles.
...
```tsx
import { RectangleEditor } from 'annota';
...
Editor for polygon annotations with vertex manipulation.
...
```tsx
import { PolygonEditor } from 'annota';
...
- Conditional vertex display in vertex editing mode
