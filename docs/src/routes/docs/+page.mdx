<script>
  ...
  import ExampleViewer from '$lib/components/examples/ExampleViewer.svelte';
</script>

# Introduction

Annota is a **high-performance annotation framework** designed specifically for whole slide imaging (WSI) and digital pathology applications. Built with TypeScript and Svelte, it provides a comprehensive solution for creating, editing, and managing annotations on large-scale medical images.
...
<Cards num={3}>
  <Card icon={Zap} title="High Performance" href="/docs/guides/tools" />
  <Card
    icon={Palette}
  ...
  />
  <Card icon={Atom} title="Svelte First" href="/api/hooks" />
</Cards>
...
<Tabs items={["npm", "pnpm", "yarn"]}>
  <Tab>```bash npm install annota openseadragon ```</Tab>
  <Tab>```bash pnpm add annota openseadragon ```</Tab>
  <Tab>```bash yarn add annota openseadragon ```</Tab>
</Tabs>
...
<Cards num={3}>
  ...
  <Card icon={GraduationCap} title="Guides" href="/docs/guides/tools" />
  <Card icon={FileText} title="API" href="/api" />
  <Card
    icon={Lightbulb}
    title="Use Cases"
    href="/docs/use-cases/basic-viewer"
  />
  <Card icon={Gamepad2} title="Playground" href="/playground" />
  <Card icon={Building} title="Architecture" href="/docs/architecture" />
</Cards>
