...
An image overlay displaying raster data at a fixed position and size:
...
- Embedding small raster images (logos, icons, watermarks)
- Displaying processed image regions (heatmaps, overlays)
- Visual markers with custom graphics
- Any fixed-size image content

**Key features:**
- Fixed dimensions (position and size are preserved)
- Supports base64 data URLs for embedding images
- Adjustable opacity for blending with underlying content
...
```svelte
<script lang="ts">
  import { PointTool, tool } from "annota/svelte";
