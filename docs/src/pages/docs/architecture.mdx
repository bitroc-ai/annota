...
import Mermaid from '../../lib/components/md/Mermaid.tsx';
...
graph TB
    subgraph UI["UI Layer"]
        Svelte["Svelte Layer<br/>Provider, Viewer<br/>tool(), annotations()"] ~~~ React["React Layer<br/>AnnotaProvider, AnnotaViewer<br/>useTool(), useAnnotations()"]
    end
...
Thin wrappers providing framework-friendly access to core APIs:

**Svelte Integration:**
- **Functions**: `tool()`, `annotations()`, `selection()`, `getAnnotator()` subscribe to store changes
- **Components**: `<Viewer>`, `<Annotator>`, `<ContextMenu>`
- **Provider**: `<AnnotaProvider>` for dependency injection
...
**Why separate?** Svelte and React are just two possible UI bindings. Core logic lives in framework-agnostic modules.
...
```typescript
store.observe((event) => {
  // Svelte functions update reactively
...
- **UI**: Provide Svelte/React bindings
