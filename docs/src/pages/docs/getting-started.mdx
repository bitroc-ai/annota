...
- **Svelte** 5+ or **React** 18+ or 19+ in your project
- Basic knowledge of Svelte runes or React hooks
...
- **Svelte** or **React**: For component-based UI integration

The framework follows a clean separation of concerns:

```
Your Svelte/React App
    ↓
AnnotaProvider (Context)
    ↓
AnnotaViewer / Viewer (OpenSeadragon)
    ↓
...
3. **Add** `AnnotaViewer` (React) or `Viewer` (Svelte) component to display your image
4. **Initialize** the `Annotator` overlay
5. **Use** `tool()` (Svelte) or `useTool()` (React) to activate annotation tools
6. **Access** annotations via `annotations()` (Svelte) or `useAnnotations()` (React)
