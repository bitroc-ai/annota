...
```svelte
<script lang="ts">
  import { getAnnotator, editing } from 'annota/svelte';
  import { getEditorConfig } from 'annota';

  const annotator = getAnnotator();
  const { startEditing } = editing();

  // Enter vertex editing mode on double-click
  annotator?.viewer?.addHandler('canvas-double-click', (event) => {
    const annotation = annotator?.getAnnotationAt(event.position);
    if (annotation) {
...
```svelte
<script lang="ts">
  import { contextMenu, editing, getAnnotator } from 'annota/svelte';
  import { getEditorConfig } from 'annota';
  ...
  function handleEditVertices() {
    if ($menuState.annotation) {
      const editorConfig = getEditorConfig($menuState.annotation);
      if (editorConfig?.supportsVertexEditing) {
...
<ContextMenu>
  {#if $menuState.annotation}
    <ContextMenuItem label="Edit Vertices" onclick={handleEditVertices} />
  {/if}
</ContextMenu>
```
