...
- **Svelte functions**: Easy integration with Svelte components
...
The `history()` function provides complete history state and controls:

```svelte
<script lang="ts">
  import { history, getAnnotator } from "annota/svelte";
...
<div class="history-controls">
  ...
  <span class="history-info">
    {$historyState.undoSize} undos, {$historyState.redoSize} redos
  </span>
...
#### Simple Undo/Redo Checks

For simple use cases, check history state:
...
{#if $historyState.canUndo}
  <button onclick={() => getAnnotator()?.undo()}>Undo</button>
{/if}

{#if $historyState.canRedo}
  <button onclick={() => getAnnotator()?.redo()}>Redo</button>
{/if}
```
...
function SimpleHistoryControls() {
  ...
  return (
    <div>
      <button onClick={() => annotator?.undo()} disabled={!canUndo}>
        Undo
      </button>
      <button onClick={() => annotator?.redo()} disabled={!canRedo}>
        Redo
      </button>
    </div>
  );
