...
```svelte
<script lang="ts">
  import { loadMaskPolygons, getAnnotator } from "annota/svelte";
...
<button onclick={handleLoad}>Load Mask</button>
```
...
function MaskImporter() {
  ...
  const loadMask = async (imageId: string) => {
    setLoading(true);
    try {
      const annotations = await loadMaskPolygons(`/masks/${imageId}.png`, {
        color: "#FFFF00",
        fillOpacity: 0.3,
        strokeWidth: 2,
      });
...
async function loadAllAnnotations(files: string[]) {
  const results = await Promise.all(files.map((file) => loadH5Masks(file)));
...
- [API](/api) - Complete loader API documentation
