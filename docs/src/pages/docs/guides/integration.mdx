...
// 3. Detect edge from point click
...
```svelte
<script lang="ts">
  import { AnnotaProvider, Viewer as AnnotaViewer, Annotator, annotations, getAnnotator } from "annota/svelte";
  ...
  onMount(async () => {
    ...
    loadedAnnotations = await loadAnnotations();
  });
...
<AnnotaProvider slideId="my-slide">
  ...
  <div>
    ...
    {#each $allAnnotations as annotation (annotation.id)}
      <div
        onclick={() => {
          annotator?.state.selection.select(annotation.id);
        }}
      >
...
```tsx
import { AnnotaProvider, AnnotaViewer, Annotator, useAnnotations } from "annota";
...
function App() {
  ...
  return (
    <AnnotaProvider slideId="my-slide">
      ...
      />
      {viewer && <Annotator viewer={viewer} />}
      <AnnotationList />
    </AnnotaProvider>
  );
}
...
function AnnotationItem({ id }: { id: string }) {
  ...
  return (
    <div
      onClick={() => {
        // Select annotation
        annotator?.state.selection.select(id);
      }}
    >
      {annotation.properties?.class}: {annotation.shape.type}
    </div>
